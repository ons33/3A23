...
THRESHOLD = 50
FAIL_CI = False  # <-- Change to False to warn only, True to fail CI
REPORT_FILE = "ai_detection_report.txt"
...
naming_pattern = re.compile(r'\b([a-z]+[A-Z][a-zA-Z0-9]*)\b')
hardcoded_string_pattern = re.compile(r'["\'].*?["\']')
try_catch_pattern = re.compile(r'\btry\b|\bcatch\b|\bfinally\b', flags=re.IGNORECASE)
...
# Run detector inside try-except
try:
    ...
    if score < THRESHOLD:
        ...
        if FAIL_CI:
            report_lines.append("⚠️ Code may be AI-generated. Failing CI.")
            raise RuntimeError("AI likelihood too high, failing CI")
        else:
            report_lines.append("⚠️ Code may be AI-generated. Warning only, CI continues.")
    ...
    print(f"\nControlled AI detection error: {e}")
    # exit(1) only if FAIL_CI is True
    if FAIL_CI:
        exit(1)
